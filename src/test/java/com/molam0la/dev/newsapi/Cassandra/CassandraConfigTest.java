...
class CassandraConfigTest {
    ...
    void getKeyspaceName() {
        assertEquals("gnews_api", cassandraConfig.getKeyspaceName());
    }

    @Test
    void getContactPoints() {
        assertEquals("127.0.0.1", cassandraConfig.getContactPoints());
    }

    @Test
    void getPort() {
        assertEquals(9042, cassandraConfig.getPort());
    }
}